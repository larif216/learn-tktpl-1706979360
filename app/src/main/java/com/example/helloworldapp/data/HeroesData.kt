...
class HeroesData {
    ...
    fun getListData(): List<Hero> {
        val heroes = ArrayList<Hero>()
        for (heroData in data) {
            val hero = Hero()
            hero.name = heroData[0]
            hero.description = heroData[1]
            hero.photo = heroData[2]
            heroes.add(hero)
        }
        return heroes
    }
